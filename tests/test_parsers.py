...
def test_good_config_filename():
    fn = 'tests/inputs/test_parsers/test1_config.yml'
    state = State()
    assert psr.load_config(state, fn) == True


def test_bad_memory_filename():
    fn = 'tests/inputs/test_parsers/non-existant-mem-file.mem'
    state = State()
    assert psr.init_memory(state, fn) == False


def test_out_of_range_address():
    fn = 'tests/inputs/test_parsers/bad_out_of_range.mem'
    state = State()
    assert psr.init_memory(state, fn) == False


def test_non_numeric_entry():
    fn = 'tests/inputs/test_parsers/bad_non_numeric.mem'
    state = State()
    assert psr.init_memory(state, fn) == False


def test_good_mem_init():
    fn = 'tests/inputs/test_parsers/good_test.mem'
    state = State()
...
def test_bad_register_filename():
    fn = 'tests/inputs/test_parsers/bad-reg-filename.reg'
    state = State()
    assert psr.init_registers(state, fn) == False


def test_bad_register_name():
    fn = 'tests/inputs/test_parsers/bad_invalid_name.reg'
    state = State()
    assert psr.init_registers(state, fn) == False


def test_bad_int_value():
    fn = 'tests/inputs/test_parsers/bad_int_value.reg'
    state = State()
    assert psr.init_registers(state, fn) == False


def test_good_reg_inti():
    fn = 'tests/inputs/test_parsers/good_test.reg'
    state = State()
...
def test_parse_instructions_good():
    fn = 'tests/inputs/test_parsers/inst1.asm'
    state = State()
